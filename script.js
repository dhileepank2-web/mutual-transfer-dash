...
async function professionalSync() {
    ...
    try {
        ...
        if (hasChanges) {
            ...
            loadMyActivity(); // Refresh my activity as well
...
// NEW: Helper function for robust date formatting
...
// NEW: Renders My Recent Activity using the new design system
async function loadMyActivity() {
    const container = $('#myActivityList').empty();
    if (!MY_PHONE) {
        container.html('<div class="text-center p-4 text-muted border rounded-24">Please log in to view your activity.</div>');
        return;
    }
    ...
    try {
        ...
        activities.forEach((act, i) => {
            const time = formatDisplayDate(act.date);
            const action = act.action.replace('_', ' ');
        ...
        container.html('<div class="text-center p-4 text-danger border rounded-24">Failed to load your activity.</div>');
